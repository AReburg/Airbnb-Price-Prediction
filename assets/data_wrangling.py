...
def parse_input(text):
    ...
    gdf = gpd.GeoDataFrame(df, geometry=gpd.GeoSeries.from_xy([lon], [lat], crs=get_local_utm_crs()))
    return gdf
...
def geo_coordinates(df):
    ...
    df = df.to_crs(get_local_utm_crs())
    return df
...
def main(df, parameters, names):
    ...
    for name, i in zip(names, parameters):
        tree = get_tree(i)
        df[name] = df.apply(lambda row: find_points_closeby(tree, (row.geometry.y, row.geometry.x)), axis=1)
